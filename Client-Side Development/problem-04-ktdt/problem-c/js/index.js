...
let losses = huskyGames2016.filter(huskiesLost);
console.log(losses);


//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method
//to extract the opponent names!
let oppLosses = losses.map(extractOpponent);
console.log(oppLosses);
...
function forEach(huskiesLost) {
  let losses = huskyGames2016.filter(huskiesLost);
  for (let x in losses) {
    console.log(losses[x].opponent + " at " + losses[x].home + ", " +  losses[x].opponent_score + " to " + losses[x].home_score);
  }
}
forEach(huskiesLost);
...
let numFumbles = [];
numFumbles = huskyGames2016.filter(fumble);
...
let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, fumble);
...
let lostByFumbles = huskyGames2016.filter(fumbledAndLostFilter);
console.log(lostByFumbles);
